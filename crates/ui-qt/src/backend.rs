...
fn flatpak_installed_scope(app_id: &str) -> &'static str {
    ...
    if in_scope("--user") && !in_scope("--system") {
        "--user"
    } else {
        "--system"
    }
...
pub struct SoftwareBackend {
    ...
    pollOp: qt_method!(
        fn pollOp(&mut self) {
            ...
            let changed =
                self.opRunning != running || self.opResult != result || self.opProgress != progress;
    ...
    pollDetail: qt_method!(
        fn pollDetail(&mut self) {
    ...
    readDetail: qt_method!(
        fn readDetail(&mut self) -> QString {
        ...
        fn loadHome(&mut self) {
            ...
            std::thread::spawn(move || {
                ...
                let result = rt.block_on(async {
                    ...
                    let installed_rpm =
                        scenter_packages::get_installed_packages().unwrap_or_default();
                    ...
                    if let Some(arr) = json["picks"].as_array_mut() {
                        mark_installed(arr);
                    }
                    if let Some(arr) = json["popular"].as_array_mut() {
                        mark_installed(arr);
                    }
                    if let Some(arr) = json["updated"].as_array_mut() {
                        mark_installed(arr);
                    }
                    if let Some(arr) = json["new"].as_array_mut() {
                        mark_installed(arr);
                    }
                    json
                });
    ...
    homeDataLoaded: qt_method!(
        fn homeDataLoaded(&mut self) -> QString {
            std::fs::read_to_string(log_path())
                .unwrap_or_default()
                .into()
        }
    ...
    // ── Installed page ────────────────────────────────────────────────────────
    loadInstalled: qt_method!(
        fn loadInstalled(&mut self) {
    ...
    // ── App detail lookup ─────────────────────────────────────────────────────
    loadAppById: qt_method!(
        fn loadAppById(&mut self, app_id: QString) {
    ...
    loadAddons: qt_method!(
        fn loadAddons(&mut self, app_id: QString, source_type: QString) {
            let app_id = app_id.to_string();
            let source_type = source_type.to_string();
            log_activity(&format!(
                "Detail: load add-ons for {app_id} (source={source_type})"
            ));
            let ready = Arc::new(AtomicBool::new(false));
    ...
    pollAddons: qt_method!(
        fn pollAddons(&mut self) {
    ...
    readAddons: qt_method!(
        fn readAddons(&mut self) -> QString {
    ...
    loadFlatpakRuntimes: qt_method!(
        fn loadFlatpakRuntimes(&mut self) {
    ...
    pollRuntimes: qt_method!(
        fn pollRuntimes(&mut self) {
    ...
    readRuntimes: qt_method!(
        fn readRuntimes(&mut self) -> QString {
            std::fs::read_to_string(runtimes_log_path())
    ...
    readStartupFilePath: qt_method!(
        fn readStartupFilePath(&mut self) -> QString {
    ...
    fileKind: qt_method!(
        fn fileKind(&mut self, path: QString) -> QString {
            let p = path.to_string().to_lowercase();
            if p.ends_with(".rpm") {
                return "rpm".into();
            }
            if p.ends_with(".flatpak") {
                return "flatpak".into();
            }
            if p.ends_with(".flatpakref") {
                return "flatpakref".into();
            }
            if p.ends_with(".appimage") {
    ...
    getLocalFileInfo: qt_method!(
        fn getLocalFileInfo(&mut self, path: QString, kind: QString) -> QString {
    ...
    installLocalFile: qt_method!(
        fn installLocalFile(
            ...
            log_activity(&format!(
                "Install local file: {path} (kind={kind}, action={action})"
            ));
            self.start_op();
            let shared = self.get_shared();
            std::thread::spawn(move || {
                ...
                let ok = match kind.as_str() {
                    "rpm" => {
                        ...
                        {
                            Box::new(scenter_packages::reinstall_local_rpm_stream(
                                &pkg_name, &path,
                            ))
                        } else {
    ...
    // ── AppImage settings ─────────────────────────────────────────────────────
    saveAppImageSettings: qt_method!(
        fn saveAppImageSettings(
            &mut self,
            id: QString,
            update_source: QString,
            update_url: QString,
            update_pattern: QString,
    ...
    loadCategory: qt_method!(
        fn loadCategory(&mut self, category: QString) {
    ...
    loadSource: qt_method!(
        fn loadSource(&mut self, source: QString) {
    ...
    // ── Updates ───────────────────────────────────────────────────────────────
    checkUpdates: qt_method!(
        fn checkUpdates(&mut self) {
    ...
    rebootSystem: qt_method!(
        fn rebootSystem(&mut self) {
    ...
    upgradePackages: qt_method!(
        fn upgradePackages(&mut self) {
            ...
            std::thread::spawn(move || {
                ...
                shared
                    .result
                    .store(if exit_code == 0 { 1 } else { 2 }, Ordering::Relaxed);
                shared.running.store(false, Ordering::Relaxed);
            });
        }
    ),

    upgradePackage: qt_method!(
        fn upgradePackage(&mut self, name: QString) {
    ...
    readRepos: qt_method!(
        fn readRepos(&mut self) -> QString {
            std::fs::read_to_string(repos_log_path())
                .unwrap_or_else(|_| "[]".to_string())
                .into()
        }
    ),
    ...
    loadUnusedPackages: qt_method!(
        fn loadUnusedPackages(&mut self) {
            let ready = Arc::new(AtomicBool::new(false));
    ...
    pollUnused: qt_method!(
        fn pollUnused(&mut self) {
    ...
    readUnused: qt_method!(
        fn readUnused(&mut self) -> QString {
            std::fs::read_to_string(unused_log_path())
                .unwrap_or_else(|_| "[]".to_string())
                .into()
        }
    ),
    ...
    setRepoEnabled: qt_method!(
        fn setRepoEnabled(&mut self, id: QString, enabled: bool) {
            let id = id.to_string();
            log_activity(&format!(
                "{} repository: {id}",
    ...
    addCopr: qt_method!(
        fn addCopr(&mut self, owner_project: QString) {
            let owner_project = owner_project.to_string().trim().to_string();
            if owner_project.is_empty() || !owner_project.contains('/') {
                let _ = std::fs::write(
                    log_path(),
                    "Invalid COPR spec. Use owner/project (e.g. tofan79/software-center).\n",
                );
                return;
            }
    ...
    removeCopr: qt_method!(
        fn removeCopr(&mut self, owner_project: QString) {
            let owner_project = owner_project.to_string().trim().to_string();
    ...
    cleanDnfCache: qt_method!(
        fn cleanDnfCache(&mut self) {
    ...
    removeUnusedPackages: qt_method!(
        fn removeUnusedPackages(&mut self) {
            log_activity("Remove unused packages (dnf autoremove)");
            self.start_op();
            let shared = self.get_shared();
            std::thread::spawn(move || {
                let _ =
                    std::fs::write(log_path(), "Removing unused packages (dnf autoremove)...\n");
                let mut exit_code = 1i32;
    ...
    cleanFlatpakUnused: qt_method!(
        fn cleanFlatpakUnused(&mut self) {
    ...
    cleanAppImageCache: qt_method!(
        fn cleanAppImageCache(&mut self) {
    ...
    upgradeFlatpak: qt_method!(
        fn upgradeFlatpak(&mut self, id: QString, source: QString, remote: QString) {
            let id = id.to_string();
            let source = source.to_string();
            let remote = remote.to_string();
            log_activity(&format!(
                "Update Flatpak: {id} (source={source}, remote={remote})"
            ));
            self.start_op();
            let shared = self.get_shared();
            std::thread::spawn(move || {
                ...
                match source.as_str() {
                    ...
                    "flatpak" => {
                        ...
                        let scope = if scenter_flatpak::is_user_remote(r) {
                            "--user"
                        } else {
                            "--system"
                        };
                ...
                match cmd.stdout(Stdio::piped()).stderr(Stdio::piped()).spawn() {
                    Ok(mut child) => {
                        ...
                        let crawl = std::thread::spawn(move || loop {
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            if !shared_crawl.running.load(Ordering::Relaxed) {
                                break;
                            }
                            ...
                            if cur < 95 {
                                shared_crawl
                                    .progress
                                    .store((cur + 1).min(95), Ordering::Relaxed);
                            }
                        });
                    ...
                    scenter_appimages::update_appimage_stream(&id, &download_url, &new_version)
                {
                    if let Some(code) = line.strip_prefix("__done__") {
                        ...
                        if let Ok(p) = pct.trim().parse::<i32>() {
                            let cur = shared.progress.load(Ordering::Relaxed);
                            if p > cur {
                                shared.progress.store(p.min(95), Ordering::Relaxed);
                            }
                        }
                    } else if !line.is_empty() {
                        append_log(&line);
    ...
    installApp: qt_method!(
        fn installApp(
            ...
            remote: QString,
            app_name: QString,
            icon_path_hint: QString,
            icon_url_hint: QString,
            user_remote: bool,
        ) {
            ...
            let shared = self.get_shared();
            log_activity(&format!(
                "Install start: {id} (source={source}, remote={remote}, user={user_remote})"
            ));
            std::thread::spawn(move || {
                use std::io::{BufRead, BufReader};
                use std::process::{Command, Stdio};
                ...
                if let Some(mut child) = child {
                    ...
                    let stderr_thread = stderr
                        .map(|s| std::thread::spawn(move || for _ in BufReader::new(s).lines() {}));
                            ...
                            .spawn()
                        {
                            Ok(mut child) => {
                                ...
                                let stderr_t = stderr.map(|s| {
                                    std::thread::spawn(
                                        move || {
                                            for _ in BufReader::new(s).lines() {}
                                        },
                                    )
                                });
    ...
    // ── System status ─────────────────────────────────────────────────────────
    loadSystemStatus: qt_method!(
        fn loadSystemStatus(&mut self) {
            self.start_op();
            let shared = self.get_shared();
            std::thread::spawn(move || {
    ...
    loadFlatpakRemotes: qt_method!(
        fn loadFlatpakRemotes(&mut self) {
    ...
    pollRemotes: qt_method!(
        fn pollRemotes(&mut self) {
    ...
    readRemotes: qt_method!(
        fn readRemotes(&mut self) -> QString {
            std::fs::read_to_string(remotes_log_path())
                .unwrap_or_else(|_| "{}".to_string())
                .into()
        }
    ),

    addFlatpakRemote: qt_method!(
        fn addFlatpakRemote(&mut self, name: QString, url: QString, system: bool) -> QString {
            log_activity(&format!(
                "Add flatpak remote: {name} ({url}, {})",
                if system { "system" } else { "user" }
            ));
            let (ok, msg) =
                scenter_flatpak::add_remote(&name.to_string(), &url.to_string(), system);
            if !ok {
                log_activity(&format!("Add flatpak remote FAILED: {name} — {msg}"));
            }
            serde_json::json!({ "ok": ok, "msg": msg })
                .to_string()
                .into()
    ...
    addFlathub: qt_method!(
        fn addFlathub(&mut self, system: bool) -> QString {
            log_activity(&format!(
                "Add Flathub ({})",
                if system { "system" } else { "user" }
            ));
            ...
            if !ok {
                log_activity(&format!("Add Flathub FAILED: {msg}"));
            }
            serde_json::json!({ "ok": ok, "msg": msg })
                .to_string()
                .into()
    ...
    addCosmicRemote: qt_method!(
        fn addCosmicRemote(&mut self, system: bool) -> QString {
            log_activity(&format!(
                "Add COSMIC remote ({})",
                if system { "system" } else { "user" }
            ));
            ...
            if !ok {
                log_activity(&format!("Add COSMIC remote FAILED: {msg}"));
            }
            serde_json::json!({ "ok": ok, "msg": msg })
                .to_string()
                .into()
    ...
    removeFlatpakRemote: qt_method!(
        fn removeFlatpakRemote(&mut self, name: QString, system: bool) -> QString {
            log_activity(&format!(
                "Remove flatpak remote: {name} ({})",
                if system { "system" } else { "user" }
            ));
            let (ok, msg) = scenter_flatpak::remove_remote(&name.to_string(), system);
            if !ok {
                log_activity(&format!("Remove flatpak remote FAILED: {name} — {msg}"));
            }
            serde_json::json!({ "ok": ok, "msg": msg })
                .to_string()
                .into()
    ...
    setFlatpakRemoteEnabled: qt_method!(
        fn setFlatpakRemoteEnabled(
            &mut self,
            name: QString,
            enabled: bool,
            system: bool,
            ...
            log_activity(&format!(
                "Set flatpak remote {name} enabled={enabled} ({})",
                if system { "system" } else { "user" }
            ));
            let (ok, msg) = scenter_flatpak::set_remote_enabled(&name.to_string(), enabled, system);
            if !ok {
                log_activity(&format!("Set flatpak remote FAILED: {name} — {msg}"));
            }
            serde_json::json!({ "ok": ok, "msg": msg })
                .to_string()
                .into()
    ...
    // ── Settings ─────────────────────────────────────────────────────────────
    loadSettings: qt_method!(
        fn loadSettings(&mut self) -> QString {
    ...
    saveSettings: qt_method!(
        fn saveSettings(&mut self, json: QString) {
    ...
    isCacheReady: qt_method!(
        fn isCacheReady(&mut self) -> bool {
    ...
    isOverlayUpdateRunning: qt_method!(
        fn isOverlayUpdateRunning(&mut self) -> bool {
    ...
    isLiveEnvironment: qt_method!(
        fn isLiveEnvironment(&mut self) -> bool {
    ...
    readStartHidden: qt_method!(
        fn readStartHidden(&mut self) -> bool {
    ...
    checkShowRequest: qt_method!(
        fn checkShowRequest(&mut self) -> bool {
    ...
    checkQuitRequest: qt_method!(
        fn checkQuitRequest(&mut self) -> bool {
    ...
    loadDaemonUpdateCache: qt_method!(
        fn loadDaemonUpdateCache(&mut self) {
    ...
    setPendingUpdateCount: qt_method!(
        fn setPendingUpdateCount(&mut self, count: i32) {
    ...
    loadUpdatesCache: qt_method!(
        fn loadUpdatesCache(&mut self) -> QString {
    ...
    clearUpdatesCache: qt_method!(
        fn clearUpdatesCache(&mut self) {
    ...
    pruneCacheEntry: qt_method!(
        fn pruneCacheEntry(&mut self, kind: QString, key: QString, value: QString) {
            let kind = kind.to_string();
            ...
            if let Ok(json) = std::fs::read_to_string(&cache_path) {
                if let Ok(mut val) = serde_json::from_str::<serde_json::Value>(&json) {
                    if let Some(arr) = val.get_mut(&kind).and_then(|v| v.as_array_mut()) {
                        arr.retain(|item| {
                            item[key.as_str()] != serde_json::Value::String(value.clone())
                        });
                    }
                    let total: usize = val["packages"].as_array().map(|a| a.len()).unwrap_or(0)
    ...
    checkDaemonTrigger: qt_method!(
        fn checkDaemonTrigger(&mut self) -> bool {
...
fn resolve_app_display_info(
    ...
    let name = if hint_name.is_empty() {
        cache_name
    } else {
        hint_name
    };
    let ip = if hint_icon_path.is_empty() {
        cache_path
    } else {
        hint_icon_path
    };
    let iu = if hint_icon_url.is_empty() {
        cache_url
    } else {
        hint_icon_url
    };
